...
def get_visitor_guid():
    """
    create a guid for visitor
    :return:
    """
...
def get_visitor_token():
    ...
    """
    host = localReadConfig.get_http("BASEURL")
    response = requests.get(host+get_url_from_xml('ico'))
    info = response.json()
    token = info.get("info")
    logger.debug("Create token:%s" % (token))
    return token
...
def set_visitor_guid_to_config():
    """
    set token that created for visitor to config
    :return:
    """
    token_v = get_visitor_guid()
    localReadConfig.set_headers("guid", token_v)


def get_value_from_return_json(json_data, name1, name2):
    """
    get value by key
    :param json_data:
    :param name1:
